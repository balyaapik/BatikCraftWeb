...
def gateway_config():
    try:
        return PaymentGatewaySetting.objects.get(
            provider=PaymentGatewaySetting.Provider.MIDTRANS
        )
...
def is_enabled() -> bool:
    ...
    if config:
        return bool(config.enabled and config.server_key)
...
def enabled_payments() -> list[str]:
    ...
    if config:
        return [
            value.strip()
            for value in config.allowed_payments.split(",")
            if value.strip()
        ]
